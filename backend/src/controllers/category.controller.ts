...
import { requireText } from '../utils/validation.ts';
import { deactivateCategory } from '../utils/catalogLifecycle.ts';
...
export async function getCategories(req, res) {
  ...
  if (req.query.search) filter.$or = ['name','slug','description'].map((field) => ({ [field]: { $regex: escapeRegex(String(req.query.search).trim()), $options: 'i' } }));
...
export async function createCategory(req, res) {
  ...
  data.name = requireText(data.name, 'Nom', { max: 120 });
...
export async function deleteCategory(req, res) {
  const category = await deactivateCategory(req.params.id, req.query.force === 'true');
  if (!category) return res.status(404).json({ message: 'Catégorie introuvable' });
  res.json({ message: 'Catégorie désactivée', categoryId: req.params.id, deactivated: true });
}
